...
*/
import {Logger, _} from '../../common/index.js';
...
export default class AbstractCommand {
  ...
  registerOptions(program) {
    program.option('--color', 'Force console logs to include ANSI color codes');
    program.option('--no-color', 'Force console logs to not include ANSI color codes (overrides --color)');
    program.option('--silent', 'Silence logging');
    return program;
  }
  ...
  collectKeyValueAndWarn(value, accumulator = {}) {
    if (value.includes('=') && !this.insecureSecretWarned) { // only warn once per invocation
      Logger.warn('Passing secrets with -s is not secure, instead use --secret-stdin, --secrets-stdin, or --secret-prompt');
    }
  ...
  validateOptions(options) {
    if (_.isNil(options.yaml) || _.isEmpty(options.yaml)) {
      Logger.info('No pipeline YAML file specified, using cix.yaml');
  ...
  configureLogger(options) {
    ...
    // PipelineService.set
    if (options.logging === 'files' || options.logging === 'file') {
      // handle default here for loggingPath, in order to allow loadCixConfig to override it
      Logger.enableFileLogging(options.logging, options.loggingPath || 'logs', options.logname || 'cix-execution.log');
    }
  }
  ...
  generateEnvironmentList(options) {
    ...
    _.forEach(options.env, (value, name) => {
      if (_.isNil(name) || _.isNil(value)) {
        Logger.warn('This environment variable is invalid, skipping it: name: ' + name + ' value: ' + value);
      } else {
