...
*/
import {ExecutionError, Logger, _} from '../common/index.js';
...
class EnvironmentService {
  ...
  setEnvironmentVar(pipelineId, environmentVar) {
    ...
    if (_.isNil(environmentVar)) {
      Logger.silly('Cannot set environment variable, parameter required.');
    ...
    if (_.isNil(environmentVar.name) || _.isNil(environmentVar.value)) {
      Logger.silly('Cannot set environment variable, parameter is malformed".');
      throw new ExecutionError('The environmentVar parameter is malformed.', 400);
    }

    Logger.debug(`Adding environment variable ${environmentVar.name} for Pipeline ${pipelineId}`);
  ...
  getEnvironmentVar(pipelineId, name) {
    if (_.isNil(name)) {
      Logger.silly('Cannot get environment variable, parameter required.');
      throw new ExecutionError('The name parameter is required.', 400);
    }
    Logger.debug(`Getting environment variable ${name} for Pipeline ${pipelineId}`);
  ...
  listEnvironmentVar(pipelineId) {
    Logger.debug(`Getting all environment variables for Pipeline ${pipelineId}`);
